...
<p>A {@link DomainObjectProperty} can be validated by putting validation annotations before the getter method of a property. Validation annotations are often used for the more basic validations. The following sections will explain the specific validation annotations that can be used.</p>
<h2>Validation methods</h2>
<p>Properties and DomainObjects can also be validated with validation methods located in the domain class so that you can do more complicated validation using code. 
</p><p>
ValidationMethods conventions:
<ul>
<li>Syntax: validate&lt;{@link DomainObjectProperty}Name or businessRuleName&gt;. E.g. for a {@link DomainObjectProperty}: validateStartDate() or for business rule: validateCustommerAlreadyExists()</li>
...
<p>
If a property is mandatory (must have a value) you can add one of the following annotations:</p>
<ul>
<li>&#64;NotNull</li>
<li>&#64;NotEmpty</li>
</ul>
TODO EXAMPLE
<h3>Validating correct format</h3>
<p>Some properties require a certain format (such as property types: date, time and currency). This format can be defined by adding the following annotations:</p>
<ul>
<li>&#64;Regexp</li>
<li>&#64;Mask</li>
</ul>
...
<p>
Add one of the following annotations if a property needs to be within a predefined range:</p>
&#64;GreatherThan
TODO SEE JSR??? TO FIND ALL ANNOTATIONS
TODO EXAMPLE

<h3>Validating business rules</h3>
<p>Business rules are needed when:</p>
<ul>
<li>Validation is more complex and can not be handled with ValidationAnnotations, but needs to be done with code.</li>
<li>Validation requires multiple property values (e.g. start date must be before end date)</li>
<li>Validation requires a different source in order to validate (e.g. check a repository if a name if unique, or ask the LanguageProvider for the localised format)</li>
</ul>
