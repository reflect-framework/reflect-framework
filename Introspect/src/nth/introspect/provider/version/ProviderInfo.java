...
public class ProviderInfo {
	...
	public ProviderInfo(Provider provider, Class<? extends Provider> providerType) {
		this.providerType = StringUtil.convertToNormalCase(providerType.getSimpleName());
		className=provider.getClass().getSimpleName();
		fullClassName = provider.getClass().getName();
		// get jar war or ear.
		try {
			URI archiveURI = provider.getClass().getProtectionDomain().getCodeSource().getLocation().toURI();
		...
		if (archiveName.endsWith(".class")) {
			...
			setArchiveFieldsForEclipseTomcatClassLoader(provider);
		} else {
			archiveTitle = provider.getClass().getPackage().getImplementationTitle();
			archiveVersion = provider.getClass().getPackage().getImplementationVersion();
			archiveCreatedBy = provider.getClass().getPackage().getImplementationVendor();
		}
