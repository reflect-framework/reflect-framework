...
import nth.introspect.excel.repository.exception.CellConversionException;
...
public abstract class ExcelReader<T> {
	...
	public ExcelReader(SheetFilter sheetFilter) {
		this.sheetFilter = sheetFilter;
	}

	public List<T> read(File excelFile, XSSFWorkbook workbook) throws Exception {
	...
	private List<Sheet> findSheets(Workbook workbook, File excelFile)
	...
	public String getStringValue(Row row, Integer columnIndex) {
	...
	public static String getFormula(Row row, int columnIndex) {
		Cell cell = row.getCell(columnIndex);
		if (cell == null) {
			return null;
		}
