...
import nth.reflect.infra.report.Section;
import nth.reflect.infra.report.TableSection;
...
public class PdfReportFactory extends ReportProvider<Document> {
	...
	// private static Font TITLE_FONT = new Font(Font.FontFamily.TIMES_ROMAN,
	// 12, Font.BOLD);
	...
	public Document createDocument(Report report) {
		...
		try {
			...
			document.setMargins(document.leftMargin(), document.rightMargin(), document.topMargin() + HEADER_HEIGHT,
					document.bottomMargin() + FOOTER_HEIGHT);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		for (Object domainObject : tableSection.getObjects()) {
			for (PropertyInfo propertyInfo : propertyInfos) {
				...
				if (value == null) {
					...
					// indent += " ";
					// }
					// pdfTable.addCell(new Phrase(indent + value.toString(),
					// SMALL_FONT));
					// } else {
	...
	public void addInvisibleChapterForHeader(Document document, Section section) {
		try {
			Paragraph paragraph = new Paragraph(section.getSectionName(),
					FontFactory.getFont(FontFactory.COURIER, 1f, BaseColor.WHITE));// hide
																					// chapter
	...
	class HeaderFooter extends PdfPageEventHelper {
		...
		int pagenumber;
		private final Phrase exportDate;
		...
		public void onEndPage(PdfWriter writer, Document document) {
			ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_LEFT, new Phrase(reportName),
					document.left(), document.top() + HEADER_HEIGHT, 0);
			ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_RIGHT, new Phrase(paragraphName),
					document.right(), document.top() + HEADER_HEIGHT, 0);
			ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_LEFT, new Phrase(exportDate),
					document.left(), document.bottomMargin() - FOOTER_HEIGHT, 0);
			ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_RIGHT,
					new Phrase(String.format("Page %d", pagenumber)), document.right(),
					document.bottomMargin() - FOOTER_HEIGHT, 0);
		}
	}
