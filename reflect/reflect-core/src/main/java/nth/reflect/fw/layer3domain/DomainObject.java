 ...
 * <li>Make sure that the referenced object type is known by the {@link ReflectFramework}: Its class must be returned by the {@link ReflectApplication#getDomainClasses()} method.</li>
 * <li>The object that creates the {@link DomainObject} needs to have a reference to
 * the {@link DomainContainer} as a constructor parameter, so that that the {@link DomainContainer#get(Class)} method can be called to get or create the {@link DomainObject} with the required dependencies injected by the {@link ReflectFramework}</li>
 * </ul>
 * </ul>
