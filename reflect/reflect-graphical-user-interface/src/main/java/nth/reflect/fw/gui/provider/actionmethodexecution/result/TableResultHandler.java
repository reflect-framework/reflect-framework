...
public abstract class TableResultHandler implements ActionMethodResultHandler {
	...
	public boolean canProcess(ActionMethodInfo actionMethodInfo) {
		return TableInfoForTableTab.supports(actionMethodInfo);
	}
	...
	public void process(UserInterfaceContainer container, Object methodOwner, ActionMethodInfo actionMethodInfo,
		...
		TableTabFilter tableTabFilter = new TableTabFilter(methodOwner, actionMethodInfo, methodParameter,
				methodResult);
		...
		if (result.isPresent()) {
			...
			Tab tableTab = createTableTab(container, methodOwner, actionMethodInfo, methodParameter);
