...
/**
 * @author Original: Shadi Shaheen Adopted by Nils ten Hoeve
 * only support the default side of the tab pane (Side.Top)
 */
public class RfxTabPaneSkin3 extends BehaviorSkinBase<TabPane, TabPaneBehavior> {
	...
	public RfxTabPaneSkin3(TabPane tabPane, UserInterfaceContainer userInterfaceContainer) {
		super(tabPane, new TabPaneBehavior(tabPane));
		this.userInterfaceContainer = userInterfaceContainer;
		
		backgroundColor = RfxColorFactory.create(RfxUtil.getPrimaryColorSet().getBackground());
		highLightColor = RfxColorFactory.create(RfxUtil.getPrimaryColorSet().getBackgroundHighLighted());
		foregroundColor1 = RfxColorFactory.create(RfxUtil.getPrimaryColorSet().getForeground1());
		foregroundColor2 = RfxColorFactory.create(RfxUtil.getPrimaryColorSet().getForeground2());
		...
		tabsContainerHolder = new AnchorPane();
				...
				tabsContainer.getChildren().add(leftDrawer);
	...
	private JFXDrawer createDrawer() {
		JFXDrawer leftDrawer = new JFXDrawer();
		RfxMainMenuList mainMenu = createMainMenu();
		leftDrawer.setSidePane(mainMenu);
		leftDrawer.setDefaultDrawerSize(200);
		leftDrawer.setOverLayVisible(false);
		leftDrawer.setResizableOnDrag(true);
		leftDrawer.prefHeightProperty().bind(tabsContainerHolder.heightProperty());
		leftDrawer.open();
		return leftDrawer;
	}

	private RfxMainMenuList createMainMenu() {
	...
	protected class HeaderContainer extends StackPane {
		...
		public HeaderContainer() {
			...
			headerBackground.setBackground(new Background(new BackgroundFill(backgroundColor, CornerRadii.EMPTY, Insets.EMPTY)));
			...
			selectedTabLine.setStroke(foregroundColor1);
			...
			menuButton=new RfxButton(nth.reflect.javafx.control.fonticon.FontAwesomeIconName.NAVICON);
			menuButton.setButtonType(ButtonType.FLAT);
			menuButton.setColorSet(RfxUtil.getPrimaryColorSet());
			menuButton.setVisible(true);
			menuButton.prefHeightProperty().bind(headersRegion.heightProperty());
			menuButton.setOnAction(this::onMenuButtonAction);
			
			getChildren().addAll(headerBackground, headersRegion, menuButton, leftControlButton, rightControlButton);
		...
		private void onMenuButtonAction(javafx.event.ActionEvent actionEvent) {
			if(leftDrawer.isShown() || leftDrawer.isShowing()) {
				leftDrawer.close();
			}	else{
				leftDrawer.open();	
			}
		...
		protected void layoutChildren() {
			...
			//TODO menuButton.showTabsMenu(true);
			...
			menuButton.resize(btnWidth, btnHeight);
			...
			positionInArea(headersRegion, startX + btnWidth+btnWidth, startY, w, h, 0, HPos.LEFT, VPos.CENTER);
			positionInArea(rightControlButton, controlStartX, controlStartY, btnWidth, btnHeight, 0, HPos.CENTER, VPos.CENTER);
			positionInArea(leftControlButton, btnWidth, controlStartY, btnWidth, btnHeight, 0, HPos.CENTER, VPos.CENTER);
			positionInArea(menuButton, 0, controlStartY-1, btnWidth, btnHeight, 0, HPos.CENTER, VPos.CENTER);
			if (!initialized) {
				double offset = 0.0;
				double selectedTabOffset = 0.0;
				double selectedTabWidth = 0.0;
	...
	protected class TabHeaderContainer extends StackPane {
		...
		public TabHeaderContainer(final Tab tab) {
			...
			rippler.setRipplerFill(highLightColor);
			...
			if (tab.isSelected()) tabText.setTextFill(foregroundColor1);
			else tabText.setTextFill(foregroundColor2);
			

			tabText.textFillProperty().addListener((o,oldVal,newVal)->{
				if(!systemChange) foregroundColor2 = (Color) newVal;
			});

			tab.selectedProperty().addListener((o, oldVal, newVal) -> {
				systemChange = true;
				if (newVal)  tabText.setTextFill(foregroundColor1);
				else tabText.setTextFill(foregroundColor2);
				systemChange = false;
			});
	...
	protected class HeaderControl extends StackPane {
		...
		public HeaderControl(ArrowPosition pos) {
			...
			arrowButton.setFill(foregroundColor1);
