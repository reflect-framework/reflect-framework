 ...
 */
public class RfxTabPaneSkin2 extends BehaviorSkinBase<TabPane, TabPaneBehavior> {
	...
	public RfxTabPaneSkin2(TabPane tabPane) {
		super(tabPane, new TabPaneBehavior(tabPane));
		
		defaultColor = RfxColorFactory.create(RfxUtil.getPrimaryColorSet().getBackground());
		ripplerColor = RfxColorFactory.create(RfxUtil.getPrimaryColorSet().getBackgroundHighLighted());
	...
	protected class HeaderContainer extends StackPane {
		...
		private HeaderControl menuButton;
...
//			menuButton=new RfxApplicationToolbarMenuButton();
//			menuButton.setVisible(true);
			menuButton = new HeaderControl(ArrowPosition.RIGHT);
			menuButton.inner.prefHeightProperty().bind(headersRegion.heightProperty());
		...
		protected void layoutChildren() {
			...
			positionInArea(menuButton, 0, controlStartY, btnWidth, btnHeight, 0, HPos.CENTER, VPos.CENTER);
			if (!initialized) {
				double offset = 0.0;
				double selectedTabOffset = 0.0;
				double selectedTabWidth = 0.0;
