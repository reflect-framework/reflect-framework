...
public class TabPaneSkin4 extends BehaviorSkinBase<TabPane, TabPaneBehavior> {
    ...
    public TabPaneSkin4(TabPane tabPane) {
   ...
   private static class StyleableProperties {
        ...
        private final static CssMetaData<TabPane,TabAnimation> OPEN_TAB_ANIMATION = 
                new CssMetaData<TabPane, TabPaneSkin4.TabAnimation>("-fx-open-tab-animation", 
            ...
            @Override public StyleableProperty<TabAnimation> getStyleableProperty(TabPane node) {
                TabPaneSkin4 skin = (TabPaneSkin4) node.getSkin();
        ...
        private final static CssMetaData<TabPane,TabAnimation> CLOSE_TAB_ANIMATION = 
                new CssMetaData<TabPane, TabPaneSkin4.TabAnimation>("-fx-close-tab-animation", 
                    new EnumConverter<TabAnimation>(TabAnimation.class), TabAnimation.GROW) {
...
//        private StackPane downArrow;
        ...
        public TabControlButtons() {            
            ...
            tabMenuBtn = new JFXButton();;
            tabMenuBtn.setButtonType(ButtonType.FLAT);
            GlyphsDude.setIcon(tabMenuBtn, FontAwesomeIconName.CALENDAR, "26px");//CLONE \uf24d
            tabMenuBtn.setVisible(isShowTabsMenu());
...
//            tabMenuBtn.getChildren().add(downArrow);
            tabMenuBtn.setOnMouseClicked(me -> {
                showPopupMenu();
            });
            ...
            inner = new StackPane() {
                @Override protected double computePrefWidth(double height) {
                    double pw;
                    double maxArrowWidth = ! isShowTabsMenu() ? 0 : snapSize( snapSize(tabMenuBtn.prefWidth(getHeight())));
                    pw = 0.0F;
                    if (isShowTabsMenu()) {
                        pw += maxArrowWidth;
                    }
                ...
                @Override protected double computePrefHeight(double width) {
                    double height = 0.0F;
                    if (isShowTabsMenu()) {
                        height = Math.max(height, snapSize(tabMenuBtn.prefHeight(width)));
                    }
                    if (height > 0) {
                ...
                @Override protected void layoutChildren() {
                    if (isShowTabsMenu()) {
                        ...
                        positionArrow(tabMenuBtn, null, x, y, w, h);
                    }
                }

                private void positionArrow(JFXButton btn, StackPane arrow, double x, double y, double width, double height) {
...
//                            /*baseline ignored*/0, HPos.CENTER, VPos.CENTER);
            ...
            inner.getChildren().add(tabMenuBtn);
        ...
        private void showPopupMenu() {
            ...
            popup.show(tabMenuBtn, Side.BOTTOM, 0, 0);            
        }
    ...
    class TabMenuItem extends RadioMenuItem {
        ...
        public TabMenuItem(final Tab tab) {
            super(tab.getText(), TabPaneSkin4.clone(tab.getGraphic()));                        
            this.tab = tab;
            setDisable(tab.isDisable());
