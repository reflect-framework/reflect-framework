...
public class HeaderBar extends HorizontalLayout {
	...
	public HeaderBar(UserInterfaceContainer userInterfaceContainer, MainWindow mainWindow) {
		...
		new StyleBuilder().setBackground(ReflectColorName.PRIMARY.BACKGROUND()).setOverflow(Overflow.HIDDEN)
				.setWidth(100, SizeUnit.PERCENT).setHeight(55, SizeUnit.PX).setPadding(5, 5, 0, 5)
				.setZIndex(MainWindow.Z_INDEX_HEADER).setFor(this);
	...
	private Span createTitle() {
		...
		Span title = new Span(applicationClassInfo.getDisplayName());
		title.setTitle(applicationClassInfo.getDescription());
		title.setId("title");
		new StyleBuilder().setColor(ReflectColorName.PRIMARY.FOREGROUND()).setOverflow(Overflow.HIDDEN)
				.setFontSize(16, SizeUnit.PT).setFont("Roboto").setFor(title);
		return title;
	}
}
